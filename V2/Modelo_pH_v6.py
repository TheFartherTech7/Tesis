...
def mostrar_resultados_modelo(df, features, modelo, poly, grado, r2, chi2, coefs):
    if len(features) == 2:
        var_names = ["X1", "Z"]
    else:
    ...
    y_pred = modelo.predict(poly.transform(df[features].values))
    n_obs = len(df)
    n_parametros = len(modelo.coef_) + 1
    ...
    f_val, p_val, df1, df2 = calcular_f_stat(df["pH"].values, y_pred, n_parametros)
