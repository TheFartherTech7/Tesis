...
def analizar_reactivo(df, reactivo_seleccionado):
    ...
    for conc in concentraciones:
        ...
        # Mostrar métricas
        # Solo mostrar R² y RMSE básicos, no todas las métricas
...
def main():
    ...
    if df is None or len(df) == 0:
        print("No se pudieron cargar los datos o no hay reactivos complejos. Verifica el archivo CSV.")
        return
    
    # Mostrar reactivos complejos disponibles
    reactivos_complejos = list(df['Reactivo'].unique())
    print(f"\nReactivos complejos disponibles: {reactivos_complejos}")
    
    # Seleccionar reactivo para analizar
    while True:
        print(f"\nReactivos complejos disponibles:")
        for i, reactivo in enumerate(reactivos_complejos, 1):
            print(f"{i}. {reactivo}")
        ...
        if seleccion.isdigit():
            idx = int(seleccion) - 1
            if 0 <= idx < len(reactivos_complejos):
                reactivo_seleccionado = reactivos_complejos[idx]
            else:
    ...
    # Calcular estadísticas globales
            ...
            # Mostrar solo estadísticas globales automáticamente
            mostrar_estadisticas_globales(estadisticas_globales, reactivo_seleccionado)
            
            # Preguntar si quiere ver estadísticas individuales
            while True:
                opcion = input("\n¿Deseas ver las estadísticas individuales por concentración? (sí/no): ").strip().lower()
                if opcion in ['si', 'sí', 's', 'yes', 'y']:
                    mostrar_estadisticas_individuales(resultados, reactivo_seleccionado)
                    break
                elif opcion in ['no', 'n']:
                    print("Estadísticas individuales omitidas.")
                    break
                else:
                    print("Por favor, responde 'sí' o 'no'.")
            
            # Preguntar si quiere ver gráficos
            while True:
                opcion = input("\n¿Deseas ver los gráficos de resultados? (sí/no): ").strip().lower()
                if opcion in ['si', 'sí', 's', 'yes', 'y']:
