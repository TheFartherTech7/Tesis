...
def busqueda_greedy_max_r2_reactivos():
    ...
    if mejor_modelo_global:
        ...
        # BUSCAR SUBCONJUNTOS MEJOR R2 QUE EL GLOBAL GREEDY
    from itertools import combinations

    print("\nBuscando subconjuntos con mejor R² que el global greedy...")
    mejor_r2_subconjunto = mejor_r2_global
    mejor_subconjunto = None

    for r in range(2, len(mejor_conjunto_global)):
        for combo in combinations(mejor_conjunto_global, r):
            r2_sub, _, _, _, _ = entrenar_y_calcular_r2_para_conjunto(set(combo))
            if r2_sub > mejor_r2_subconjunto:
                mejor_r2_subconjunto = r2_sub
                mejor_subconjunto = combo

    if mejor_subconjunto:
        print(f"\nSe encontró un subconjunto mejor que el global greedy:")
        print(f"Reactivos: {sorted(mejor_subconjunto)}")
        print(f"R²: {mejor_r2_subconjunto:.4f}")
    else:
        print("\nNo se encontró ningún subconjunto con mejor R² que el global greedy.")
