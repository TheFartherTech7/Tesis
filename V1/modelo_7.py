...
def guardar_resultados_csv(titulo, grado, r2, eq):
    guardar_csv = input("\n¿Deseas guardar los resultados a CSV? (Sí/No): ").strip().lower()
    if guardar_csv in ["sí", "si"]:
...
def graficar_superficie(df_subset, modelo, poly, grado, titulo="Modelo", cmap='viridis'):
    ...
    if input("\n¿Deseas guardar esta gráfica como imagen? (Sí/No): ").strip().lower() in ["sí", "si"]:
...
def analizar_reactivo():
    ...
    graficar_superficie(sub_df, modelo, poly, grado, titulo=reactivo)
...
def analisis_global(df_entrada, titulo="Modelo Global"):
    ...
    modelo, grado, r2, eq, poly = ajustar_modelo(X, y)
    print(f"\n--- {titulo} ---\nGrado: {grado}\nR^2: {r2:.4f}\nEcuación: {eq}")
    graficar_superficie(df_entrada, modelo, poly, grado, titulo=titulo)
